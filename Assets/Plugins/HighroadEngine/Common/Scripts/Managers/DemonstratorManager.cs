...
{
    /// <summary>
    /// 这个类允许在没有RaceManager的情况下快速启动演示场景
    /// 你只需要把它放在一个场景中，让摄像头自动绑定到车辆上
    /// 并且该载具与玩家1的输入绑定。
    /// 如果场景中有多个车辆/摄像机并且处于活动状态，GUI中会出现一个按钮
    /// 在车辆和摄像机之间切换
    /// 禁用FindCamerasInScene和FindVehiclesInScene允许您通过相机和车辆列表手动关联相机和车辆
    /// </summary>
